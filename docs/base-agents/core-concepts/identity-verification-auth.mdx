...
A trusted agent registry (ERC-8004) allows agents and services to register their identity onchain. When registered, your agent has a publicly auditable identity declaring what it does and where its endpoints live. Other agents can check this registry before interacting.

Discovery alone isn't enough. Verification (ERC-8128) happens at execution time. When your agent registers, its identity is tied to a cryptographic keypair. When your agent calls a service, the service signs its response using those keys. Your agent checks the signature against the registry. If they match, you're talking to the right endpoint.
...
<Card title="Payment &  Transactions" icon="link" href="/base-agents/core-concepts/onchain-capabilities">
